...
import static com.colasoft.opensearch.index.RandomCreateIndexGenerator.randomMappingFields;
import static com.colasoft.opensearch.test.AbstractXContentTestCase.xContentTester;
...
public class GetIndexTemplatesResponseTests extends OpenSearchTestCase {
    ...
    public void testParsingFromOpenSearchResponse() throws IOException {
        for (int runs = 0; runs < 20; runs++) {
            com.colasoft.opensearch.action.admin.indices.template.get.GetIndexTemplatesResponse esResponse =
                new com.colasoft.opensearch.action.admin.indices.template.get.GetIndexTemplatesResponse(new ArrayList<>());

            XContentType xContentType = randomFrom(XContentType.values());
            int numTemplates = randomIntBetween(0, 32);
            for (int i = 0; i < numTemplates; i++) {
                com.colasoft.opensearch.cluster.metadata.IndexTemplateMetadata.Builder esIMD =
                    new com.colasoft.opensearch.cluster.metadata.IndexTemplateMetadata.Builder(
            ...
            try (
                ...
                for (int i = 0; i < numTemplates; i++) {
                    com.colasoft.opensearch.cluster.metadata.IndexTemplateMetadata esIMD = esResponse.getIndexTemplates().get(i);
    ...
    static void toXContent(GetIndexTemplatesResponse response, XContentBuilder builder) throws IOException {
        ...
        List<com.colasoft.opensearch.cluster.metadata.IndexTemplateMetadata> serverIndexTemplates = new ArrayList<>();
        ...
        for (IndexTemplateMetadata clientITMD : clientIndexTemplates) {
            com.colasoft.opensearch.cluster.metadata.IndexTemplateMetadata.Builder serverTemplateBuilder =
                com.colasoft.opensearch.cluster.metadata.IndexTemplateMetadata.builder(clientITMD.name());
        ...
        com.colasoft.opensearch.action.admin.indices.template.get.GetIndexTemplatesResponse serverResponse =
            new com.colasoft.opensearch.action.admin.indices.template.get.GetIndexTemplatesResponse(serverIndexTemplates);
