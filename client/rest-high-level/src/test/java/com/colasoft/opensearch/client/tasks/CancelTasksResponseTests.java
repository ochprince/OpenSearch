...
package com.colasoft.opensearch.client.tasks;
...
public class CancelTasksResponseTests extends AbstractResponseTestCase<
    ...
    com.colasoft.opensearch.client.tasks.CancelTasksResponse> {
    ...
    protected CancelTasksResponseTests.ByNodeCancelTasksResponse createServerTestInstance(XContentType xContentType) {
        List<com.colasoft.opensearch.tasks.TaskInfo> tasks = new ArrayList<>();
        ...
        for (int i = 0; i < 4; i++) {
            ...
            tasks.add(
                new com.colasoft.opensearch.tasks.TaskInfo(
    ...
    @Override
    protected com.colasoft.opensearch.client.tasks.CancelTasksResponse doParseToClientInstance(XContentParser parser) throws IOException {
        return com.colasoft.opensearch.client.tasks.CancelTasksResponse.fromXContent(parser);
    }

    @Override
    protected void assertInstances(
        ByNodeCancelTasksResponse serverTestInstance,
        com.colasoft.opensearch.client.tasks.CancelTasksResponse clientInstance
    ) {

        // checking tasks
        List<TaskInfo> sTasks = serverTestInstance.getTasks();
        List<com.colasoft.opensearch.client.tasks.TaskInfo> cTasks = clientInstance.getTasks();
        Map<com.colasoft.opensearch.client.tasks.TaskId, com.colasoft.opensearch.client.tasks.TaskInfo> cTasksMap = cTasks.stream()
            .collect(Collectors.toMap(com.colasoft.opensearch.client.tasks.TaskInfo::getTaskId, Function.identity()));
        for (TaskInfo ti : sTasks) {
            com.colasoft.opensearch.client.tasks.TaskInfo taskInfo = cTasksMap.get(
                new com.colasoft.opensearch.client.tasks.TaskId(ti.getTaskId().getNodeId(), ti.getTaskId().getId())
            );
        ...
        List<com.colasoft.opensearch.client.tasks.OpenSearchException> cNodeFailures = clientInstance.getNodeFailures();
        List<String> sExceptionsMessages = serverNodeFailures.stream()
            .map(x -> com.colasoft.opensearch.client.tasks.OpenSearchException.buildMessage("exception", x.getMessage(), null))
            .collect(Collectors.toList());

        List<String> cExceptionsMessages = cNodeFailures.stream()
            .map(com.colasoft.opensearch.client.tasks.OpenSearchException::getMsg)
            .collect(Collectors.toList());
        ...
        List<com.colasoft.opensearch.client.tasks.TaskOperationFailure> cTaskFailures = clientInstance.getTaskFailures();

        Map<Long, com.colasoft.opensearch.client.tasks.TaskOperationFailure> cTasksFailuresMap = cTaskFailures.stream()
            .collect(Collectors.toMap(com.colasoft.opensearch.client.tasks.TaskOperationFailure::getTaskId, Function.identity()));
        for (TaskOperationFailure tof : sTaskFailures) {
            com.colasoft.opensearch.client.tasks.TaskOperationFailure failure = cTasksFailuresMap.get(tof.getTaskId());
