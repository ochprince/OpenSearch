...
import com.colasoft.opensearch.client.RequestConverters.EndpointBuilder;
import com.colasoft.opensearch.client.core.CountRequest;
import com.colasoft.opensearch.client.core.GetSourceRequest;
import com.colasoft.opensearch.client.core.MultiTermVectorsRequest;
import com.colasoft.opensearch.client.core.TermVectorsRequest;
import com.colasoft.opensearch.client.indices.AnalyzeRequest;
import com.colasoft.opensearch.common.CheckedBiConsumer;
...
import com.colasoft.opensearch.index.rankeval.PrecisionAtK;
import com.colasoft.opensearch.index.rankeval.RankEvalRequest;
import com.colasoft.opensearch.index.rankeval.RankEvalSpec;
import com.colasoft.opensearch.index.rankeval.RatedRequest;
import com.colasoft.opensearch.index.rankeval.RestRankEvalAction;
import com.colasoft.opensearch.index.reindex.AbstractBulkByScrollRequest;
import com.colasoft.opensearch.index.reindex.DeleteByQueryRequest;
import com.colasoft.opensearch.index.reindex.ReindexRequest;
import com.colasoft.opensearch.index.reindex.RemoteInfo;
import com.colasoft.opensearch.index.reindex.UpdateByQueryRequest;
...
import com.colasoft.opensearch.script.mustache.MultiSearchTemplateRequest;
import com.colasoft.opensearch.script.mustache.SearchTemplateRequest;
import com.colasoft.opensearch.search.Scroll;
...
import com.colasoft.opensearch.search.collapse.CollapseBuilder;
import com.colasoft.opensearch.search.fetch.subphase.FetchSourceContext;
import com.colasoft.opensearch.search.fetch.subphase.highlight.HighlightBuilder;
...
import com.colasoft.opensearch.test.RandomObjects;
...
import static com.colasoft.opensearch.client.RequestConverters.REQUEST_BODY_CONTENT_TYPE;
import static com.colasoft.opensearch.client.RequestConverters.enforceSameContentType;
import static com.colasoft.opensearch.index.query.QueryBuilders.matchAllQuery;
import static com.colasoft.opensearch.search.RandomSearchRequestGenerator.randomSearchRequest;
...
public class RequestConvertersTests extends OpenSearchTestCase {
    ...
    public void testRethrottle() {
        ...
        variants.add(
            new Tuple<String, Supplier<Request>>("_update_by_query", () -> RequestConverters.rethrottleUpdateByQuery(rethrottleRequest))
        );
        variants.add(
            new Tuple<String, Supplier<Request>>("_delete_by_query", () -> RequestConverters.rethrottleDeleteByQuery(rethrottleRequest))
        );
    ...
    public void testBulkWithDifferentContentTypes() throws IOException {
        ...
        }
        {
            ...
            IllegalArgumentException exception = expectThrows(IllegalArgumentException.class, () -> RequestConverters.bulk(bulkRequest));
            assertEquals(
                "Unsupported content-type found for request with content-type [" + xContentType + "], only JSON and SMILE are supported",
                exception.getMessage()
            );
        }
    }
            ...
            if (frequently()) {
                ...
                if (randomBoolean()) {
                    searchSourceBuilder.aggregation(
                        new TermsAggregationBuilder(randomAlphaOfLengthBetween(3, 10)).userValueTypeHint(ValueType.STRING)
                ...
                if (randomBoolean()) {
                    searchSourceBuilder.addRescorer(
                        new QueryRescorerBuilder(new TermQueryBuilder(randomAlphaOfLengthBetween(3, 10), randomAlphaOfLengthBetween(3, 10)))
                    );
                }
                if (randomBoolean()) {
    ...
    public void testEnforceSameContentType() {
        ...
        assertEquals(
            "Unsupported content-type found for request with content-type [CBOR], only JSON and SMILE are supported",
            exception.getMessage()
        );
        ...
        assertEquals(
            "Unsupported content-type found for request with content-type [YAML], only JSON and SMILE are supported",
            exception.getMessage()
        );
    ...
    public static void setRandomIndicesOptions(
        Consumer<IndicesOptions> setter,
        Supplier<IndicesOptions> getter,
    ...
    static void setRandomTimeout(TimedRequest request, TimeValue defaultTimeout, Map<String, String> expectedParams) {
        setRandomTimeout(
            s -> request.setTimeout(TimeValue.parseTimeValue(s, request.getClass().getName() + ".timeout")),
            defaultTimeout,
            expectedParams
        );
    }
    ...
    static void setRandomTimeoutTimeValue(Consumer<TimeValue> setter, TimeValue defaultTimeout, Map<String, String> expectedParams) {
        if (randomBoolean()) {
    ...
    static void setRandomWaitForActiveShards(
        Consumer<ActiveShardCount> setter,
