...
import com.colasoft.opensearch.action.support.master.ShardsAcknowledgedResponse;
import com.colasoft.opensearch.client.AbstractResponseTestCase;
...
public class CloseIndexResponseTests extends AbstractResponseTestCase<
    com.colasoft.opensearch.action.admin.indices.close.CloseIndexResponse,
    CloseIndexResponse> {

    @Override
    protected com.colasoft.opensearch.action.admin.indices.close.CloseIndexResponse createServerTestInstance(XContentType xContentType) {
        boolean acknowledged = true;
        final String[] indicesNames = generateRandomStringArray(10, 10, false, true);

        final List<com.colasoft.opensearch.action.admin.indices.close.CloseIndexResponse.IndexResult> indexResults = new ArrayList<>();
        for (String indexName : indicesNames) {
            final Index index = new Index(indexName, randomAlphaOfLength(5));
            if (randomBoolean()) {
                indexResults.add(new com.colasoft.opensearch.action.admin.indices.close.CloseIndexResponse.IndexResult(index));
            } else {
                if (randomBoolean()) {
                    acknowledged = false;
                    Exception exception = randomFrom(new IndexNotFoundException(index), new ActionNotFoundTransportException("test"));
                    indexResults.add(new com.colasoft.opensearch.action.admin.indices.close.CloseIndexResponse.IndexResult(index, exception));
                } else {
                    final int nbShards = randomIntBetween(1, 5);
                    com.colasoft.opensearch.action.admin.indices.close.CloseIndexResponse.ShardResult[] shards =
                        new com.colasoft.opensearch.action.admin.indices.close.CloseIndexResponse.ShardResult[nbShards];
                    for (int i = 0; i < nbShards; i++) {
                        com.colasoft.opensearch.action.admin.indices.close.CloseIndexResponse.ShardResult.Failure[] failures = null;
                        if (randomBoolean()) {
                            acknowledged = false;
                            int nbFailures = randomIntBetween(1, 3);
                            failures = new com.colasoft.opensearch.action.admin.indices.close.CloseIndexResponse.ShardResult.Failure[nbFailures];
                        ...
                        shards[i] = new com.colasoft.opensearch.action.admin.indices.close.CloseIndexResponse.ShardResult(i, failures);
                    }
                    indexResults.add(new com.colasoft.opensearch.action.admin.indices.close.CloseIndexResponse.IndexResult(index, shards));
                }
            }
        }

        final boolean shardsAcknowledged = acknowledged ? randomBoolean() : false;
        return new com.colasoft.opensearch.action.admin.indices.close.CloseIndexResponse(acknowledged, shardsAcknowledged, indexResults);
    }
    ...
    protected void assertInstances(
        final com.colasoft.opensearch.action.admin.indices.close.CloseIndexResponse serverInstance,
        ...
        serverInstance.getIndices().forEach(expectedIndexResult -> {
            ...
            if (expectedIndexResult.getShards() != null) {
                ...
                List<com.colasoft.opensearch.action.admin.indices.close.CloseIndexResponse.ShardResult> failedShardResults = Arrays.stream(
                    expectedIndexResult.getShards()
                )
                    .filter(com.colasoft.opensearch.action.admin.indices.close.CloseIndexResponse.ShardResult::hasFailures)
                    .collect(Collectors.toList());
                ...
                failedShardResults.forEach(failedShardResult -> {
                    ...
                    for (int i = 0; i < failedShardResult.getFailures().length; i++) {
                        com.colasoft.opensearch.action.admin.indices.close.CloseIndexResponse.ShardResult.Failure expectedFailure = failedShardResult
                            .getFailures()[i];
    ...
    private com.colasoft.opensearch.action.admin.indices.close.CloseIndexResponse.ShardResult.Failure newFailure(
        ...
        return new com.colasoft.opensearch.action.admin.indices.close.CloseIndexResponse.ShardResult.Failure(indexName, shard, exception, nodeId);
    }
}
