...
import com.colasoft.opensearch.gradle.BwcVersions
...
import com.colasoft.opensearch.gradle.plugin.PluginBuildPlugin
import com.colasoft.opensearch.gradle.plugin.PluginPropertiesExtension
...
class ClusterFormationTasks {
    ...
    static void configureDistributionDependency(Project project, String distro, Configuration configuration, String opensearchVersion) {
        ...
        if (distro.equals("integ-test-zip")) {
            ...
            if (internalBuild) {
                ...
                project.dependencies.add(
                        configuration.name,
                        "com.colasoft.opensearch.distribution.integ-test-zip:opensearch:${opensearchVersion}@zip"
                )
            }
            return
        }
    ...
    static void configureBwcPluginDependency(Project project, Object plugin, Configuration configuration, Version opensearchVersion) {
        if (plugin instanceof Project) {
            ...
            project.dependencies.add(configuration.name, "com.colasoft.opensearch.plugin:${pluginName}:${opensearchVersion}@zip")
        } else {
    ...
    static Task configureCheckPreviousTask(String name, Project project, Object depends, NodeInfo node) {
        return project.tasks.create(name: name, type: Exec, dependsOn: depends) {
            ...
            doLast {
                ...
                if (out.contains("${ext.pid} com.colasoft.opensearch.bootstrap.OpenSearch") == false) {
                    logger.error('jps -l')
                    logger.error(out)
                    logger.error("pid file: ${node.pidFile}")
                    logger.error("pid: ${ext.pid}")
                    throw new GradleException("jps -l did not report any process with com.colasoft.opensearch.bootstrap.OpenSearch\n" +
