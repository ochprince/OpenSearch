...
import com.colasoft.opensearch.ingest.ProcessorInfo;
...
public class NodeInfoStreamingTests extends OpenSearchTestCase {
    ...
    private static NodeInfo createNodeInfo() {
        ...
        if (randomBoolean()) {
            int numProcessors = randomIntBetween(0, 5);
            List<com.colasoft.opensearch.search.pipeline.ProcessorInfo> processors = new ArrayList<>(numProcessors);
            for (int i = 0; i < numProcessors; i++) {
                processors.add(new com.colasoft.opensearch.search.pipeline.ProcessorInfo(randomAlphaOfLengthBetween(3, 10)));
            }
