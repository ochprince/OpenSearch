...
import static com.colasoft.opensearch.test.StreamsUtils.copyToBytesFromClasspath;
...
public class DocumentParserTests extends MapperServiceTestCase {
    ...
    public void testParseToJsonAndParse() throws Exception {
        String mapping = copyToStringFromClasspath("/com/colasoft/opensearch/index/mapper/simple/test-mapping.json");
        ...
        BytesReference json = new BytesArray(copyToBytesFromClasspath("/com/colasoft/opensearch/index/mapper/simple/test1.json"));
    ...
    public void testSimpleParserNoTypeNoId() throws Exception {
        String mapping = copyToStringFromClasspath("/com/colasoft/opensearch/index/mapper/simple/test-mapping.json");
        DocumentMapper docMapper = createDocumentMapper(MapperService.SINGLE_MAPPING_NAME, mapping);
        BytesReference json = new BytesArray(copyToBytesFromClasspath("/com/colasoft/opensearch/index/mapper/simple/test1-notype-noid.json"));
        Document doc = docMapper.parse(new SourceToParse("test", "1", json, XContentType.JSON)).rootDoc();
        assertThat(doc.getBinaryValue(docMapper.idFieldMapper().name()), equalTo(Uid.encodeId("1")));
        assertThat(doc.get(docMapper.mappers().getMapper("name.first").name()), equalTo("fred"));
    }
