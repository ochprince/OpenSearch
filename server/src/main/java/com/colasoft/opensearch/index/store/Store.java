...
import com.colasoft.opensearch.index.engine.CombinedDeletionPolicy;
...
import static com.colasoft.opensearch.index.store.Store.MetadataSnapshot.loadMetadata;
import static com.colasoft.opensearch.indices.replication.SegmentReplicationTarget.REPLICATION_PREFIX;
...
public class Store extends AbstractIndexShardComponent implements Closeable, RefCounted {
     ...
     * policy ( current major - 1). See also {@link #readSegmentInfosExtendedCompatibility(Directory, com.colasoft.opensearch.Version)}.
     *
     * @throws IOException if the index is corrupted or the segments file is not present
     */
     ...
     * @throws IOException if the index is corrupted or the segments file is not present
     */
    private static SegmentInfos readSegmentInfosExtendedCompatibility(Directory directory, com.colasoft.opensearch.Version minimumVersion)
        throws IOException {
        try {
    ...
    public static final class MetadataSnapshot implements Iterable<StoreFileMetadata>, Writeable {
        ...
        static LoadedMetadata loadMetadata(SegmentInfos segmentInfos, Directory directory, Logger logger, boolean ignoreSegmentsFile)
            ...
            if (maxVersion == null) {
                maxVersion = com.colasoft.opensearch.Version.CURRENT.minimumIndexCompatibilityVersion().luceneVersion;
            }
     ...
     *
     * @see MetadataSnapshot#recoveryDiff(com.colasoft.opensearch.index.store.Store.MetadataSnapshot)
     *
     * @opensearch.internal
     */
    ...
    public interface OnClose extends Consumer<ShardLock> {
        OnClose EMPTY = new OnClose() {
            /**
             * This method is called while the provided {@link com.colasoft.opensearch.env.ShardLock} is held.
    ...
    /**
     * Returns a {@link com.colasoft.opensearch.index.seqno.SequenceNumbers.CommitInfo} of the safe commit if exists.
     */
