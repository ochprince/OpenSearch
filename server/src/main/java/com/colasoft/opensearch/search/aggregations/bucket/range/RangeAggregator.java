...
import com.colasoft.opensearch.search.aggregations.bucket.BucketsAggregator;
...
public class RangeAggregator extends BucketsAggregator {
    ...
    public InternalAggregation buildEmptyAggregation() {
        ...
        List<com.colasoft.opensearch.search.aggregations.bucket.range.Range.Bucket> buckets = new ArrayList<>(ranges.length);
        for (int i = 0; i < ranges.length; i++) {
            Range range = ranges[i];
            com.colasoft.opensearch.search.aggregations.bucket.range.Range.Bucket bucket = rangeFactory.createBucket(
                range.key,
                range.from,
                range.to,
                0,
